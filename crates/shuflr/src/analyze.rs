...
pub struct AnalysisReport {
    ...
    /// Max KL divergence of any sampled frame's byte distribution vs the global.
    pub byte_kl_max: f64,
...
pub fn run(reader: &mut SeekableReader, sample_chunks: usize, seed: u64) -> Result<AnalysisReport> {
    ...
    // KL divergence per frame against global.
    ...
    for hist in &per_frame_hists {
        ...
        for i in 0..256 {
            ...
            let p = hist[i] as f64 / total as f64;
            let q = global_p[i];
            // If q is 0 we'd get infinity; skip (nothing in global for this
            // byte, which shouldn't happen if the frame contributed to global).
            if q > 0.0 {
                kl += p * (p / q).ln();
            }
        }
    ...
    let (kl_max, kl_mean) = if kls.is_empty() {
        (0.0, 0.0)
    } else {
        let sum: f64 = kls.iter().sum();
        let max = kls.iter().cloned().fold(f64::NEG_INFINITY, f64::max);
        (max, sum / kls.len() as f64)
    };

    // Record-length coefficient of variation across sampled frames.
    let reclen_cv = coefficient_of_variation(&per_frame_mean_len);

    let verdict = if kl_max > BYTE_KL_THRESHOLD_UNSAFE || reclen_cv > RECLEN_CV_THRESHOLD_UNSAFE {
        Verdict::Unsafe
    } else {
        Verdict::Safe
    };
